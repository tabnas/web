...
which catches the class of bug where a plugin stopped applying without reporting an error.
...
<a id="test-the-grammar-not-just-the-parse"></a>

## Test grammar structure and parse results
...
and are not yours. A test that asserts the listing turns a silent regression (actions that stop firing because a mark was renamed) into a failure:
