...
and Node strips types without a flag only from 22.18, while a build dependency
(`undici`, via `unifont`) declares `node >=22.19.0`. `npm run dev` serves the pages but not the Pagefind
