...
// The numbers .vale.ini records for each demoted rule, checked against a
// live run rather than remembered. Reads the same built pages the prose
// gate does, so it needs `npm run build` first.
...
const GUIDE = Path.join(REPO, 'STYLE-GUIDE.md')
...
function gatedDocs() {
  return [...prepare().outputs, Path.join(REPO, 'README.md')]
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (0 === wrong.length) {
    ...
    process.stderr.write('vale-counts: the recorded counts are not what Vale reports.\n  ' +
      wrong.join('\n  ') +
      '\n\nRe-measure with: node tools/vale-counts.mjs --write\n')
    process.exitCode = 1
  }
}

export { report, blocks, measure }
