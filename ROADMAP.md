...
| `/about` | What the project is, who builds it, how it is funded, what state it is in |
| `/contact` | Which channel suits which kind of message; the maintainer's address |
| `/api` | The machine-readable surface, rendered from the same OpenAPI document `/openapi.json` serialises |
| `/404` | The designed not-found page. A program gets the same recovery routes as markdown or JSON — see `AGENTS.md § The request Worker` |
...
- ~~`robots.txt`~~ — shipped, generated by `src/pages/robots.txt.ts`, with the
  sitemap and a `Content-Signal` line granting `search`, `ai-input` and
  `ai-train`. **Submitting the sitemap to search console is still to do**, and
  it is the part that actually moves brand-name search.

Three questions this raised were decided rather than deferred, and are
recorded here because each is a policy rather than a fix, and reversing one is
a decision too:

- **`ai-train=yes`.** Training use is granted, not merely undeclared. The
  reasoning is in `src/pages/robots.txt.ts`: MIT-licensed documentation for a
  project whose thesis is that agents should be able to write grammars is not
  content to withhold from training. A granted signal is not easily walked
  back.
- **The Organization's address is the country and nothing more** — `IE`, in
  `ORG.address` in `consts.ts`. Enough to answer "where is this from" without
  publishing a street for a one-maintainer project.
- **`www.tabnas.dev` 301s to the apex**, in `src/worker.ts`. The canonical
  tags said so already; now the host does.
