...
- ~~`robots.txt`~~ — shipped, generated by `src/pages/robots.txt.ts`, with the
  sitemap and a `Content-Signal` line granting `search` and `ai-input`.
  **Submitting the sitemap to search console is still to do**, and it is the
  part that actually moves brand-name search.
- `ai-train` is deliberately undeclared in `robots.txt`, which under the
  Content Signals Policy neither grants nor restricts it. That one is a rights
  decision for the maintainer, not a default to assume — decide it.
- The Organization JSON-LD has no `address`, because the project has never
  published one. `ORG.address` in `consts.ts` is null and the field is omitted;
  fill it in and it appears. Until then the identity graph is incomplete for
  anything checking business legitimacy.
- `www.tabnas.dev` serves 200 alongside the apex rather than redirecting to it.
  The canonical tags already point at `tabnas.dev`, so search engines
  consolidate, but a 301 would make it unambiguous.
